...
import { ArrowLeft, Crown, Repeat, ShieldCheck, Sparkles, TrendingUp } from 'lucide-react';
...
// here derives from stored scores, so it sharpens every finalized GW.
export default async function LeagueStatsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const session = await readSession();
  if (!session) redirect(`/?next=/league/${id}/stats`);
  ...
  return (
    <div className="reveal space-y-4 py-4 lg:mx-auto lg:max-w-2xl">
      ...
      <h1 className="text-center font-display text-4xl">Stats</h1>
